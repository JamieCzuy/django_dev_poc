FROM python:3.10-slim-buster AS python_base
...
FROM python_package_installer as django_project_update_requirements
...
FROM python_package_installer as django_project_packages
...
FROM python_base as django_project
# Copy the app's packages (lib and bin)
COPY --from=app_packages /tmp/django_project/python_env /usr/local
COPY ./django_project /django_project
WORKDIR /
# TODO - Run using a runner gunicorn or uvicorn
...
FROM python_package_installer as api_tests_update_requirements
RUN pip install pip-tools
...
FROM python_package_installer as api_tests_packages
...
FROM python_package_installer as e2e_tests_update_requirements
RUN pip install pip-tools
...
FROM python_package_installer as e2e_tests_packages
...
FROM python_base as unit_tests
